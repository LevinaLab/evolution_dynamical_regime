repeats=10
cores=10

# subfolder="plot_many_functions"
subfolder_add="b1_dynamic_range_break_to_eat"
subfolder="sim-$(date '+%Y%m%d-%H%M%S')_parallel_$subfolder_add"
# date="date +%s"
# subfolder="$date$subfolder"

command="python3 train.py -b 1 -v_eat_max 0.005 -g 4001 -t 2000 -compress -noplt -rec_c 20 -c 5 -subfolder ${subfolder} -n Run_{1}"



parallel --bar --eta -j${cores} ${command} ::: $(seq ${repeats})

# seq 5 | parallel -n0 ${command} 